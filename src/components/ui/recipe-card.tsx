...
export function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  return (
    <div className="group flex flex-col bg-surface border border-border rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:border-primary/50 transition-all duration-300">
      <Link href={`/recipes/${recipe._id}`} className="relative h-48 w-full overflow-hidden block bg-background">
        {recipe.imageUrl ? (
          <Image
            src={imageUrl}
            alt={recipe.title}
      ...
      <div className="flex flex-col flex-grow p-4">
        ...
        <p className="text-sm text-foreground-muted line-clamp-2 mb-4 flex-grow">
        ...
        <div className="flex items-center justify-between text-xs text-foreground-muted mb-4">
          ...
          <div className="flex items-center gap-1.5 px-2 py-0.5 bg-background text-foreground-muted border border-border rounded-md font-medium">
        ...
        <Link 
          ...
          className="w-full text-center py-2 px-4 bg-primary hover:bg-primary/95 text-white font-medium rounded-lg transition-colors duration-300 text-sm"
        >
          View Details
        </Link>
      </div>

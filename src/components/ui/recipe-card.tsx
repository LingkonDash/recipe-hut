  ...
  return (
    <div className={flat ? "group flex flex-col h-full w-full" : "group flex flex-col h-full bg-surface border border-border rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:border-primary/50 transition-all duration-300"}>
      <Link href={`/recipes/${recipe._id}`} className="relative aspect-[4/3] w-full overflow-hidden block bg-background">
        ...
        <div className="absolute top-2 right-2 bg-surface/90 backdrop-blur-sm px-2 py-1 rounded text-xs font-semibold text-accent flex items-center gap-1">
      ...
      <div className="flex flex-col flex-grow p-4">
        ...
        <Link href={`/recipes/${recipe._id}`} className="hover:text-primary transition-colors">
          <h3 className="text-lg font-bold text-foreground mb-1 line-clamp-2 h-12 leading-6">{recipe.title}</h3>
        </Link>
        
        <p className="text-sm text-foreground-muted line-clamp-2 mb-4 h-10 leading-5">
          {recipe.shortDescription}
        </p>
        
        <div className="flex items-center justify-between text-xs text-foreground-muted mb-4 h-6 border-t border-border/50 pt-3">
          <div className="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            <span>{recipe.prepTimeMinutes + recipe.cookTimeMinutes} min</span>
          </div>
          <div className="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>
            <span>{recipe.calories} cal</span>
          </div>
          <div className="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5 text-accent"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
        ...
        <Link 
          ...
          className="w-full text-center py-2 px-4 bg-primary hover:bg-primary/95 text-white font-medium rounded-lg transition-colors duration-300 text-sm mt-auto"
        >
          View Details
        </Link>
      </div>

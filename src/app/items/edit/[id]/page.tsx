...
import { EditRecipeFormWrapper } from './edit-recipe-form-wrapper';

export const metadata: Metadata = {
  title: 'Edit Recipe | Recipe Hut',
  description: 'Modify and update your recipe details.',
};
...
export default async function EditRecipePage({
  ...
  try {
    ...
    console.error('Error fetching recipe server-side:', err);
    redirect('/items/manage');
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <div className="relative overflow-hidden border-b border-zinc-100 dark:border-zinc-800 bg-gradient-to-br from-[var(--primary)]/5 via-[var(--accent)]/5 to-[var(--secondary)]/5">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          ...
          <nav className="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-6">
            ...
            <span>/</span>
            <span className="text-[var(--foreground)] font-medium">Edit Recipe</span>
          </nav>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
        <EditRecipeFormWrapper recipe={recipe} />
      </div>
    </div>
  );
}
